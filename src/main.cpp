...
DEFINE_double(confidence_threshold, 0.1, "Only store detections with score higher than the threshold.");  //置信度

int main(int argc, char** argv) {
        string model_file = "../srcmodel/MobileNetSSD_deploy.prototxt";
        string weights_file = "../srcmodel/MobileNetSSD_final.caffemodel";
...
*/
        ...
        cv::Mat img = cv::imread("../test_image/03647.jpg");
        //MobileNet-SSD检测
        std::vector<vector<float> > detections = detector.Detect(img);
        detector.Postprocess(img, confidence_threshold, detections);
